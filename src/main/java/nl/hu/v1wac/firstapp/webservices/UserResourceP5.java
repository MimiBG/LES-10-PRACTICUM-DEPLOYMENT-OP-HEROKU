...
public class UserResourceP5 {
	...
	public Response addAccount( @Context SecurityContext sc,
								...
								@FormParam("role") String rol,
								@FormParam("email") String email,
								@FormParam("telefoonnummer") int telefoonnummer,
								@FormParam("adres") String adres) throws SQLException {
		boolean role = sc.isUserInRole("user");
		System.out.println("HEEFT ROL USER: " + role + " (in UserResource @POST)");
		if (role) {
			Account account = service.addNewAccount(username, password, rol, email, telefoonnummer, adres);
